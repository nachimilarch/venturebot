...
// events defaults to ["message.received"]. Available: message.received, message.delivered, message.read, message.failed
router.post('/', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      ...
      [tenantId, name.trim(), url.trim(), secret, eventsStr]
    );
    ...
    res.status(201).json({
      ...
      data: {
        ...
        is_active: true,
        secret,
        note: 'Save your signing secret now — it will not be shown again. Use it to verify X-Vaartabot-Signature on incoming events.',
      },
    });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
...
// Update url, name, events, or toggle is_active.
...
router.delete('/:id', async (req, res) => {
  try {
    const [result] = await pool.execute(
      ...
      [parseInt(req.params.id), req.apiTenant.tenantId]
    );
...
// Sends a test ping to the registered URL.
router.post('/:id/test', async (req, res) => {
  try {
    ...
    const payload = JSON.stringify({
      event: 'test',
      tenant_id: req.apiTenant.tenantId,
